...
def crossover_b(mating_pool, width, height, gene_size):
    ...
    while (len(genes) < gene_size):
        if (i > gene_size):
            adjacent_genes = Util.bound_adjacent(genes, width, height)
            genes.append(random.choice(adjacent_genes))
            if (len(genes) > gene_size):
                print('gene size over')
        else:
            genes2 = list(set(genes2).difference(set(genes)))
...
def crossover_c(mating_pool, width, height, gene_size):
    ...
    while (len(genes) < gene_size):
        if (i > gene_size):
            print('infinite loop let')
            genes.append(random.choice(Util.bound_adjacent(genes, width, height)))
        if len(genes) < gene_size:
            genes2 = list(set(genes2).difference(set(genes)))
            adjs_in_genes1 = list(set(Util.bound_adjacent(genes, width, height)).intersection(set(genes1)))
...
def mutate(genes, width, height, rate=0.1, mutate_option='improvement', fit_option='f(PAR)'):
...
def mutate_old0920(genes, width, height, rate = 0.1):
    if(random.random() >= rate):
        return genes
    else:
        newgene = genes.copy()
