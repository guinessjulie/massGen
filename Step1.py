...
class GenArchiPlan(unittest.TestCase):
    ...
    def test_generate(self):
        pos = self.generate(self.width, self.height, self.num_cells)
        tupPos = [tuple((p.x, p.y)) for p in pos]
    ...
    def test1(self):
        ...
        num_cells = Util.get_num_cells(width, height, floorAreaRatio)
        pos =  self.generate(width, height, num_cells) # pos = fitness.floor = genes
        tupPos = [tuple((p.x, p.y)) for p in pos]
        print('tupPos: ',tupPos)
        vertices = Util.trace(tupPos)
        print('vertices list: ',vertices)
        self.get_fitness(pos, width, height, num_cells) # todo: revert no, no no no no fitness has already options so do not botgher the option here
