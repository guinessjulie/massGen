...
import time
from genop import mutate0915, selected_fitnesses, selection, crossover_c, crossover_b, crossover_overwrap,crossover_random, calcFitness, crossoverFour, mutate, update_fitness, crossover_a
...
class GenArchiPlan(unittest.TestCase):
    ...
    def initial_population(self, pop_size, targetpath='.\\results\\'):
        ...
        while len(pops) < pop_size:
            ...
            # landgrid = self.generate_new_simple0914(self.width, self.height, self.num_cells, len(pops))  # todo: for temp only 0825 # method A
            landgrid = self.generate_origin0909(self.width, self.height, self.num_cells, len(pops)) #method C
            # landgrid = self.generate_gero0909(self.width, self.height, self.num_cells, len(pops)-1)  # todo: for temp only 0825
    ...
    def reproduce0909(self, pops, generation,fitness_filename, targetpath='./results/'):
        ...
        object_fits  = {mass.pop_id: mass.fitness._fits[fit_option] for mass in pops}
        ...
        while len(new_pops) < self.pop_size:
            ...
            # mutated_gene = mutate(child_genes, self.width, self.height, mutation_rate)
            mutated_gene = mutate0915(child_genes, self.width, self.height, mutation_rate, mutate_option = mutate_option, fit_option = fit_option)
...
### It is important to get fast local improvement
    ...
    def generate_gero(self, width, height, num_cells, pop_id = 0):
        ...
        genes = self.get_first(width, height)
        # grid = LandGrid(genes, width, height) #grid가 있어야 동적으로 gene 생성 가능
        curIdx = 0
    ...
    def generate_gero0909(self, width, height, num_cells, pop_id = 0): #method B
        # genes = Util.select_random_position(width, height)
        genes = self.get_first(width, height)
        # grid = LandGrid(genes, width, height) #grid가 있어야 동적으로 gene 생성 가능
        curIdx = 0
        ...
        while len(genes) < num_cells:
            ...
            while not gene_set:
                ...
                i += 1
            # if len(gene_set)  < 1:
            #     gene_set = Util.bound_adjacent(genes, width, height) # get rid of it and replace
    ...
    def generate_origin(self, width, height, num_cells, pop_id = 0): # method B
        genes = list(Util.select_random_position(width, height))
        sample = []
        while len(genes) < num_cells:
            while not sample:
    ...
    def generate_origin0909(self, width, height, num_cells, pop_id = 0): # method B
        genes = self.get_first(width, height)
        # genes = list(Util.select_random_position(width, height))
        sample = []
        prob = [width / (width+height), height / (width+height)]
